...
async function TrashesLeaveRequests({
  ...
  const scheduleTrashes = await RemoveModel.find(query)
    .populate([
      {
        path: "deletedBy",
        select: "name",
  ...
  return (
    <div className="h-full w-full overflow-y-auto">
      {scheduleTrashes.length <= 0 && (
        <ShowNoText>No Trashes found</ShowNoText>
      )}
      {scheduleTrashes.map((remove, index) => {
        return (
      ...
      {count > scheduleTrashes.length && (
        <LastElement count={count} data={scheduleTrashes.length} />
      )}
    </div>
  );
}
