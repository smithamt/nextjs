...
async function TrashesLeaveRequests({
  ...
  const leaveRequestTrashes = await RemoveModel.find(query)
    .populate([
      {
        path: "deletedBy",
        select: "name",
  ...
  return (
    <div className="h-full w-full overflow-y-auto">
      {leaveRequestTrashes.map((remove, index) => {
        return (
            ...
            <div className="flex justify-between">
              ...
              <div className="flex space-x-2">
                <HoverCard>
                  ...
                  <HoverCardContent className="w-[200px] mx-4">Delete Trash</HoverCardContent>
                </HoverCard>
                <HoverCard>
                  <HoverCardTrigger>
                  ...
                  <HoverCardContent className="w-[200px] mx-4">Restore Leave Request</HoverCardContent>
                </HoverCard>
              </div>
            </div>
      ...
      {count > leaveRequestTrashes.length && (
        <LastElement count={count} data={leaveRequestTrashes.length} />
      )}
    </div>
  );
}
