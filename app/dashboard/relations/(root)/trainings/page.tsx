...
async function RelationsClearances({
  ...
  const empQuery = await getEmployeeQuery(searchParams, "trainings");
  ...
  const data = (await ClearanceModel.find()
    .sort({ createdAt: -1 })
      ...
      <div className="w-full max-w-[700px] mx-auto h-full">
        <div className="py-2 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            <Link
              href={"/dashboard/relations/trainings/create"}
              className={cn(buttonVariants({ variant: "default" }))}
            >
              New Training
            </Link>
            <ExportBtn data={JSON.stringify(data)} title={"trainings"} />
          </div>
        </div>
            ...
            >
              <CardHeader className="p-2 flex flex-row justify-between">
                <EmployeeProfile
                  ...
                  to={`/dashboard/relations/tranings/${l._id}`}
                />
