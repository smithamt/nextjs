...
import { ClearanceType, TitleType } from "@/types";
...
async function RelationsClearances({
  ...
  const empQuery = await getEmployeeQuery(searchParams, state as TitleType);
  ...
  const clearances = (await SalaryIncrement.find()
    .sort({ createdAt: -1 })
    .populate("employee", "name nickname profile employeeId")
    .limit(limit)) as ClearanceType[];

  const count = await SalaryIncrement.countDocuments();

  return (
    <div className="w-full h-full">
      <div className="w-full max-w-[700px] mx-auto h-full overflow-y-auto">
        <div className="py-2 flex items-center justify-between">
          <p>Increments {count}</p>
          <Link
            href={"/dashboard/relations/increments/create"}
            ...
            >
              <CardHeader className="p-2 flex flex-row justify-between">
                <EmployeeProfile
                  ...
                  to={`/dashboard/relations/increments/${l._id}`}
                />
