  ...
  return (
    ...
    >
      ...
      <div className="p-10">
        ...
        {leaveRequest.leaves.map((leave, index) => {
          ...
          return (
            <Fragment key={index}>
              <div
                className="text-center border py-2 my-2 font-bold"
                style={{ border: `1px solid ${leave.leave?.color?.back}` }}
              >
                LEAVE TYPE - 假期类型 - ခွင့်အမျိုးအစား ({leave.leave?.name})
              </div>
              <div className="grid grid-cols-2 gap-2">
                <p title="Leave Start Date">
                  {dayjs(leave.from).format(formatDate)}
                </p>
                <p title="Leave End Date">
                  {dayjs(leave.to).format(formatDate)}
                </p>
                <p title="Reporting Date (on duty)">
                  {dayjs(leave.from).add(totalDays, "d").format(formatDate)}
                </p>
                <p title="Total Days">
